...
mqttLogger.error('Failed to subscribe', { topic: 'invalid/topic' }, new Error('Invalid topic'));

// Transform/data processing logging
transformLogger.debug('Applying transform', { type: 'extractValue', input: { value: 42 } });
transformLogger.info('Transform completed', { result: 42, duration: '2ms' });
transformLogger.error('Transform failed', { script: 'invalid code', input: 'data' }, new Error('Syntax error'));

// Widget-related logging
widgetLogger.info('Widget mounted', { widgetId: 'temp-gauge', type: 'gauge' });
widgetLogger.warn('Widget data missing', { widgetId: 'pressure-chart', binding: 'mqtt://sensors/pressure' });
widgetLogger.error('Widget render error', { widgetId: 'flow-rate', props: {} }, new Error('Missing required prop'));

// Dashboard-level logging
dashboardLogger.info('Dashboard loaded', { componentCount: 7, mqttConnected: true });
dashboardLogger.debug('Component updated', { componentId: 'widget-123', newValue: 'active' });

// System-level logging (errors, startup, shutdown)
systemLogger.info('Application started', { version: '1.0.0', environment: 'development' });
systemLogger.error('Critical system error', { component: 'mqtt-client', action: 'reconnect' }, new Error('Connection lost'));
...
logger.warn('validation', 'Invalid input detected', { field: 'email', value: 'invalid@' });
...
export function MyComponent() {
  useEffect(() => {
    ...
    return () => {
      widgetLogger.debug('Component unmounted', { componentName: 'MyComponent' });
    };
  }, []);
  ...
  const handleAction = async () => {
    try {
      ...
      widgetLogger.error('Action failed', { 
        action: 'fetchData' 
      }, error as Error);
    }
  };
...
async function connectToMqtt() {
  ...
  try {
    ...
    mqttLogger.error('MQTT connection failed', { 
